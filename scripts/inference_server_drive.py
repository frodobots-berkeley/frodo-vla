...
from palivla.inference_drive import run_inference, pil_to_base64, make_sharding
...
def gen_action():
    ...
    actions = run_inference(model, prompt, obs, config)

    print(actions)

    actions[...,0] =  actions[...,0]*MAX_CURVE
    actions[...,1] =  actions[...,1]*MAX_SPEED
    ...
    if viz is not None:
        ...
        run.log(viz)
    response = jsonify(actions=action.tolist())
    return response
