...
gcloud alpha compute tpus tpu-vm ssh $TPU_VM_NAME --zone=us-central2-b --worker=all --command="tmux kill-session -t tpc_$TPU_VM_NAME; tmux new -s l -d 'bash -li /tmp/launch.sh $API_KEY'"